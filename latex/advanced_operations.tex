...
input graphs. Pseudocode is given below.
...
%    new_graph = Graph()
%
%    // Add all pairs of start states to the queue. For each pair
...
%        (u1, u2) = queue.pop()
%        // Get all pairs of outgoing arcs from states u1 and u2
%        // with matching labels.
%        for arc1, arc2 in label_matching_arcs(u1, u2) {
%            v1 = arc1.dst_node()
%            v2 = arc2.dst_node()
%
%            // If we haven't visited the state pair (v1, v2) yet, then
%            // add it to the queue and create a new state in the
%            // intersected graph. The new state is an accept state if
%            // both v1 and v2 are accept states.
%            if not_visited((v1, v2)) {
%                queue.push((v1, v2))
%                new_graph.add_state((v1, v2))
%            }
%
%            // Add an arc to the new graph from (u1, u2) to (v1, v2)
%            // with the label from the matching arcs and a weight
%            // given by the sum of the weights of the matching arcs.
%            label = arc1.label()
...
Some of the steps of the intersect algorithm on the two input graphs above are
shown in the sequence of graphs in figure~\ref{fig:intersect_steps}. The states
explored at each step are highlighted in red. The intersected graph is the
third graph on the right which is constructed over the steps of the algorithm.
...
The pseudocode does not handle $\epsilon$ transitions. The challenge with
including $\epsilon$ transitions in intersect and compose is discussed in
section~\ref{sec:epsilon_intersect}.

Another potential issue with the pseudocode above is that the output graph it
constructs is not \emph{trim}. An automata is trim if every state in the graph
is part of a path which starts in a start state and terminates in an accept
state.  In short, the graph does not have any useless states. In the standard
terminology, a state is said to be \emph{accessible} if it can be reached from
a start state and \emph{coaccessible} if an accept state can be reached from
it. A graph is trim if every state is both accessible and coaccessible.

Every state in the pseudocode above will be accessible, but not necessarily
coaccessible, so the graph may not be trim. However, the graph will still be
correct, so this is primarily an issue of representation size. With some
additional work the constructed graph can be kept trim during the operation of
the algorithm. Another alternative is to construct a trim graph from the
non-trim graph as a post-processing step. Note, in the figures above showing
some steps of the intersect algorithm, we removed dead-end states in the
intersected graph for clarity; however, the algorithm in the pseudocode does
not do this.
...
\begin{figure}
    ...
    \caption{The intersected graph accepts the strings $aab$, $abc$, and $bcc$.}
...
\begin{proof}[\unskip\nopunct]
The composition is given in the graph below. As an example, the first input
graph transduces $abc$ to $xyz$ and the second input transduces $xyz$ to $abb$,
$abc$, $bbb$, and $bbc$. Thus, the composed graph should transduce $abc$ to all
four of $abb$, $abc$, $bbb$, and $bbc$. You can verify this in the graph below.

\begin{figure}
    \centering
    \includegraphics[scale=\dotscale]{figures/compose_example}
    \caption{The composition of the two graphs above.}
...
\begin{proof}[\unskip\nopunct]
The intersected graph is below. The $\epsilon$ transition is included for
clarity, though it could be removed and states $1$ and $2$ collapsed yielding
an equivalent graph.

\begin{figure}
    \centering
    \includegraphics[scale=\dotscale]{figures/epsilon_intersect_example}
    \caption{The intersection of the two graphs above.}
...
below, each of which has an $\epsilon$ transition.
...
If we compute the intersection of the above two graphs, optionally following
$\epsilon$ transitions as we encounter them, then we end up with the graph
below.

\begin{figure}
    \centering
    \includegraphics[scale=\dotscale]{figures/epsilon_intersect_both}
    \caption{Naively following $\epsilon$ transitions as we encounter them when
    intersecting the two graphs above results in an incorrect graph. The graph
    admits too many paths for the sequence $ab$, and hence the score of that
...
\begin{figure}
    ...
    the two graphs above. Notice that only one of the three resultant paths
    should be retained in the intersected graph.}
...
The graph above has three possible paths from the start states to the accept
state. The paths and their scores are:
...
Viterbi path as a simple linear graph as below. Note the Viterbi path may not
be unique--multiple paths could all attain the Viterbi score. The forward
score is the \emph{log-sum-exp} over all the path scores, in this case $\LSE(4.6,
5.3, 3.5) = 5.81$. The forward score will always be larger than the Viterbi
score. However, the two will converge as the difference between Viterbi score
and the second highest scoring path increases.

\begin{figure}
    \centering
    \includegraphics[scale=\dotscale]{figures/viterbi_path}
    \caption{The Viterbi path for the graph above. The score of the Viterbi
    path is the Viterbi score, in this case $5.3$.}
...
shown graphically below. In the figure, the Viterbi score of state $3$ is the
...
terminating at a given state. Suppose, as in the graph below, we have a state
$v$ for which we want to compute the score $s_v$. Suppose also that $v$ has
only one incoming arc from state $u$. Three paths from a start state terminate
at state $u$ with the given scores $p_1$, $p_2$, and $p_3$. We can extend each
of the three paths from $u$ to $v$ by adding the arc between them, so there are
three paths terminating at $v$ as well.
...
\begin{figure}
    ...
    \caption{The Viterbi path of the above graph.}
